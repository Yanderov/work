...
public abstract class AbstractNetworkIF implements NetworkIF {
   ...
   private final Supplier vmMacAddrProps;
   ...
   protected static List getNetworkInterfaces(boolean includeLocalInterfaces) {
      ...
      return includeLocalInterfaces ? interfaces : (List)((Stream)getAllNetworkInterfaces().stream().parallel()).filter((ni) -> !isLocalInterface(ni)).collect(Collectors.toList());
   }
