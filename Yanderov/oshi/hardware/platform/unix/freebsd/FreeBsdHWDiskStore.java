   ...
   public boolean updateAttributes() {
      ...
      for(String line : output) {
         ...
         if (split.length >= 7 && split[0].equals(this.getName())) {
            diskFound = true;
            this.reads = (long)ParseUtil.parseDoubleOrDefault(split[1], (double)0.0F);
            this.writes = (long)ParseUtil.parseDoubleOrDefault(split[2], (double)0.0F);
            this.readBytes = (long)(ParseUtil.parseDoubleOrDefault(split[3], (double)0.0F) * (double)1024.0F);
            this.writeBytes = (long)(ParseUtil.parseDoubleOrDefault(split[4], (double)0.0F) * (double)1024.0F);
            this.currentQueueLength = ParseUtil.parseLongOrDefault(split[5], 0L);
            this.transferTime = (long)(ParseUtil.parseDoubleOrDefault(split[6], (double)0.0F) * (double)1000.0F);
            this.timeStamp = now;
         }
      }
   ...
   public static List getDisks() {
      ...
      for(String line : iostat) {
         ...
         if (split.length > 6 && devices.contains(split[0])) {
            ...
            store.reads = (long)ParseUtil.parseDoubleOrDefault(split[1], (double)0.0F);
            store.writes = (long)ParseUtil.parseDoubleOrDefault(split[2], (double)0.0F);
            store.readBytes = (long)(ParseUtil.parseDoubleOrDefault(split[3], (double)0.0F) * (double)1024.0F);
            store.writeBytes = (long)(ParseUtil.parseDoubleOrDefault(split[4], (double)0.0F) * (double)1024.0F);
            store.currentQueueLength = ParseUtil.parseLongOrDefault(split[5], 0L);
            store.transferTime = (long)(ParseUtil.parseDoubleOrDefault(split[6], (double)0.0F) * (double)1000.0F);
