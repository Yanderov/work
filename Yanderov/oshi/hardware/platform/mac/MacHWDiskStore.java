...
public final class MacHWDiskStore extends AbstractHWDiskStore {
   ...
   private MacHWDiskStore(String name, String model, String serial, long size, DiskArbitration.DASessionRef session, Map mountPointMap, Map cfKeyMap) {
   ...
   private boolean updateDiskStats(DiskArbitration.DASessionRef session, Map mountPointMap, Map cfKeyMap) {
            ...
            if (drive != null) {
               if (!drive.conformsTo("IOMedia")) {
                  ...
                  if (parent != null && (parent.conformsTo("IOBlockStorageDriver") || parent.conformsTo("AppleAPFSContainerScheme"))) {
                     ...
                     Pointer readTimeResult = statistics.getValue((PointerType)cfKeyMap.get(MacHWDiskStore.CFKey.READ_TIME));
                     Pointer writeTimeResult = statistics.getValue((PointerType)cfKeyMap.get(MacHWDiskStore.CFKey.WRITE_TIME));
                  ...
                  if (serviceIterator != null) {
                     for(IOKit.IORegistryEntry sdService = IOKit.INSTANCE.IOIteratorNext(serviceIterator); sdService != null; sdService = IOKit.INSTANCE.IOIteratorNext(serviceIterator)) {
                        ...
                        String mountPoint = (String)mountPointMap.getOrDefault(partBsdName, "");
            ...
            if (disk != null) {
               ...
               if (diskInfo != null) {
                  ...
                  if (!"Disk Image".equals(model)) {
                     CoreFoundation.CFStringRef modelNameRef = CFStringRef.createCFString(model);
                     CoreFoundation.CFMutableDictionaryRef propertyDict = CF.CFDictionaryCreateMutable(CF.CFAllocatorGetDefault(), new CoreFoundation.CFIndex(0L), (Pointer)null, (Pointer)null);
   ...
   private static Map mapCFKeys() {
      Map<CFKey, CoreFoundation.CFStringRef> keyMap = new EnumMap(CFKey.class);

      for(CFKey cfKey : MacHWDiskStore.CFKey.values()) {
   ...
   private static enum CFKey {
      ...
      // $FF: synthetic method
      private static CFKey[] $values() {
         return new CFKey[]{IO_PROPERTY_MATCH, STATISTICS, READ_OPS, READ_BYTES, READ_TIME, WRITE_OPS, WRITE_BYTES, WRITE_TIME, BSD_UNIT, LEAF, WHOLE, DA_MEDIA_NAME, DA_VOLUME_NAME, DA_MEDIA_SIZE, DA_DEVICE_MODEL, MODEL};
      }
   }
}
