...
final class LinuxSensors extends AbstractSensors {
   ...
   private static final boolean IS_PI = queryCpuTemperatureFromVcGenCmd() > (double)0.0F;
   private final Map sensorsMap = new HashMap();
      ...
      if (IS_PI) {
         ...
         if (tempStr != null) {
            ...
            if (tempStr.contains("hwmon")) {
               ...
               if (millidegrees > 0L) {
                  return (double)millidegrees / (double)1000.0F;
               }
               ...
               if (count > 0) {
                  return (double)sum / ((double)count * (double)1000.0F);
               }
   ...
   private static double queryCpuTemperatureFromVcGenCmd() {
      ...
      return tempStr.startsWith("temp=") ? ParseUtil.parseDoubleOrDefault(tempStr.replaceAll("[^\\d|\\.]+", ""), (double)0.0F) : (double)0.0F;
   }
   ...
   public double queryCpuVoltage() {
      if (IS_PI) {
         ...
         return voltageStr != null ? (double)FileUtil.getIntFromFile(String.format("%s1_input", voltageStr)) / (double)1000.0F : (double)0.0F;
      }
   }
   ...
   private static double queryCpuVoltageFromVcGenCmd() {
      ...
      return voltageStr.startsWith("volt=") ? ParseUtil.parseDoubleOrDefault(voltageStr.replaceAll("[^\\d|\\.]+", ""), (double)0.0F) : (double)0.0F;
   }
}
