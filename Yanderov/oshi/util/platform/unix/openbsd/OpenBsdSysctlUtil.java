...
public final class OpenBsdSysctlUtil {
   ...
   public static int sysctl(int[] name, int def) {
      ...
      label50: {
         try {
            ...
            label46: {
               try {
                  if (0 == OpenBsdLibc.INSTANCE.sysctl(name, name.length, p, size, (Pointer)null, size_t.ZERO)) {
                     var5 = p.getInt(0L);
                     break label46;
                  }
   ...
   public static String sysctl(int[] name, String def) {
      ...
      label60: {
         ...
         label59: {
            try {
               if (0 != OpenBsdLibc.INSTANCE.sysctl(name, name.length, (Pointer)null, size, (Pointer)null, size_t.ZERO)) {
                  LOG.warn("Failed sysctl call: {}, Error code: {}", name, Native.getLastError());
                  var10 = def;
                  break label60;
               }
   ...
   public static boolean sysctl(int[] name, Structure struct) {
      ...
      label27: {
         try {
            if (0 != OpenBsdLibc.INSTANCE.sysctl(name, name.length, struct.getPointer(), size, (Pointer)null, size_t.ZERO)) {
      ...
      label37: {
         ...
         label36: {
            try {
               ...
               if (0 != OpenBsdLibc.INSTANCE.sysctl(name, name.length, m, size, (Pointer)null, size_t.ZERO)) {
                  LOG.error("Failed sysctl call: {}, Error code: {}", name, Native.getLastError());
                  m.close();
                  var7 = null;
                  break label36;
