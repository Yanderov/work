...
public class MacFileSystem extends AbstractFileSystem {
   ...
   private static final List FS_PATH_EXCLUDES = FileSystemUtil.loadAndParseFileSystemConfig("oshi.os.mac.filesystem.path.excludes");
   private static final List FS_PATH_INCLUDES = FileSystemUtil.loadAndParseFileSystemConfig("oshi.os.mac.filesystem.path.includes");
   private static final List FS_VOLUME_EXCLUDES = FileSystemUtil.loadAndParseFileSystemConfig("oshi.os.mac.filesystem.volume.excludes");
   private static final List FS_VOLUME_INCLUDES = FileSystemUtil.loadAndParseFileSystemConfig("oshi.os.mac.filesystem.volume.includes");
   ...
   private static List getFileStoreMatching(String nameToMatch, boolean localOnly) {
      ...
      if (numfs > 0) {
         ...
         if (session == null) {
            ...
            CoreFoundation.CFStringRef daVolumeNameKey = CFStringRef.createCFString("DAVolumeName");
            ...
            for(int f = 0; f < numfs; ++f) {
               ...
               if ((!localOnly || (flags & 4096) != 0) && (path.equals("/") || !PSEUDO_FS_TYPES.contains(type) && !FileSystemUtil.isFileStoreExcluded(path, volume, FS_PATH_INCLUDES, FS_PATH_EXCLUDES, FS_VOLUME_INCLUDES, FS_VOLUME_EXCLUDES))) {
                  ...
                  if (nameToMatch == null || nameToMatch.equals(name)) {
                     ...
                     String moreOptions = (String)OPTIONS_MAP.entrySet().stream().filter((e) -> ((Integer)e.getKey() & flags) > 0).map(Map.Entry::getValue).collect(Collectors.joining(","));
   ...
   public long getMaxFileDescriptorsPerProcess() {
      return (long)SysctlUtil.sysctl("kern.maxfilesperproc", 0);
   }
