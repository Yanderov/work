...
public class MacInternetProtocolStats extends AbstractInternetProtocolStats {
   ...
   private Supplier establishedv4v6 = Memoizer.memoize(NetStat::queryTcpnetstat, Memoizer.defaultExpiration());
   private Supplier tcpstat = Memoizer.memoize(MacInternetProtocolStats::queryTcpstat, Memoizer.defaultExpiration());
   private Supplier udpstat = Memoizer.memoize(MacInternetProtocolStats::queryUdpstat, Memoizer.defaultExpiration());
   private Supplier ipstat = Memoizer.memoize(MacInternetProtocolStats::queryIpstat, Memoizer.defaultExpiration());
   private Supplier ip6stat = Memoizer.memoize(MacInternetProtocolStats::queryIp6stat, Memoizer.defaultExpiration());
   ...
   private static InternetProtocolStats.IPConnection queryIPConnection(int pid, int fd) {
      ...
      label59: {
         int lport;
         label58: {
            InternetProtocolStats.IPConnection var11;
            label57: {
               try {
                  ...
                  if (si.size() == ret && si.psi.soi_family == 2 || si.psi.soi_family == 30) {
                     ...
                     lport = ParseUtil.bigEndian16ToLittleEndian(ini.insi_lport);
                     int fport = ParseUtil.bigEndian16ToLittleEndian(ini.insi_fport);
                     var11 = new InternetProtocolStats.IPConnection(type, laddr, lport, faddr, fport, state, si.psi.soi_qlen, si.psi.soi_incqlen, pid);
                     break label57;
                  }
