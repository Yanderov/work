...
public final class ThreadInfo {
   ...
   public static List queryTaskThreads(int pid) {
      String pidStr = " " + pid + " ";
      List<ThreadStats> taskThreads = new ArrayList();
      List<String> psThread = (List)ExecutingCommand.runNative("ps -awwxM").stream().filter((s) -> s.contains(pidStr)).collect(Collectors.toList());
      int tid = 0;

      for(String thread : psThread) {
         Matcher m = PS_M.matcher(thread);
         if (m.matches() && pid == ParseUtil.parseIntOrDefault(m.group(1), -1)) {
            double cpu = ParseUtil.parseDoubleOrDefault(m.group(2), (double)0.0F);
   ...
   public static class ThreadStats {
      ...
      public ThreadStats(int tid, double cpu, char state, long sTime, long uTime, int pri) {
         ...
         this.upTime = (long)((double)(uTime + sTime) / (cpu / (double)100.0F + 5.0E-4));
         switch (state) {
            case 'I':
            case 'S':
