package fun.Yanderov.commands.manager;
...
import fun.Yanderov.commands.defaults.DefaultCommands;
...
import fun.Yanderov.utils.client.managers.api.command.exception.CommandUnhandledException;
import fun.Yanderov.utils.client.managers.api.command.exception.ICommandException;
...
public class CommandRepository implements ICommandManager {
   ...
   public Stream tabComplete(String prefix) {
      ...
      return args.isEmpty() ? (new TabCompleteHelper()).addCommands(Yanderov.getInstance().getCommandRepository()).filterPrefix(label).stream() : this.tabComplete(pair);
   }
