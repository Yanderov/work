...
public class ModeElement implements IElementable {
   ...
   public void button(double x, double y, double width, double height, double mouseX, double mouseY, double xStart, double yStart, double xEnd, double yEnd, DrawContext drawContext, int click) {
      ...
      BuiltText text = (BuiltText)Builder.text().font(FontManager.MONTSERRAT.get()).text(this.modeSetting.getName()).color(colorText).size(7.5F).thickness(0.05F).build();
      text.render(matrix, x, y + 3.5D);
      BuiltRectangle rectangle = Builder.rectangle().size(new SizeState(width, (double)(this.modeSetting.getOffset() + 16.0F))).color(new QuadColorState(ColorUtil.setAlpha(this.animation, new Color(0, 0, 0, 3)))).radius(new QuadRadiusState(3.0F)).smoothness(1.15F).build();
      rectangle.render(matrix, x, y + 16.0D);
      BuiltBorder border = (BuiltBorder)Builder.border().size(new SizeState(width, (double)(this.modeSetting.getOffset() + 16.0F))).color(new QuadColorState(ColorUtil.colorAlpha(Color.WHITE, (int)(3.0D * this.animation)))).radius(new QuadRadiusState(3.0F)).thickness(0.01F).smoothness(0.65F, 0.65F).build();
      border.render(matrix, x, y + 16.0D);
      this.offsetX = 0.0F;
      this.offsetY = 0.0F;

      for(Mode m : this.modeSetting.getModes()) {
         String name = m.name().substring(0, 1).toUpperCase() + m.name().substring(1).toLowerCase();
         if ((double)(this.offsetX + FontManager.SUISSEINTMEDIUM.get().getWidth(name, 7.0F) + 5.0F) > width - 8.0D) {
            this.offsetY += 14.0F;
            this.offsetX = 0.0F;
         }

         this.rendereMultiboxButtonButton(m, x + 2.0D + (double)this.offsetX, y + 18.0D + (double)this.offsetY, (double)(FontManager.SUISSEINTMEDIUM.get().getWidth(name, 7.0F) + 5.0F), 12.0D, mouseX, mouseY, xStart, yStart, xEnd, yEnd, click, matrix);
   ...
   private void rendereMultiboxButtonButton(Mode mode, double x, double y, double width, double height, double mouseX, double mouseY, double xStart, double yStart, double xEnd, double yEnd, int click, Matrix4f matrix) {
      boolean isSelected = mode.name().equals(this.modeSetting.getValue());
      ...
      String displayName = mode.name().substring(0, 1).toUpperCase() + mode.name().substring(1).toLowerCase();
      boolean animationFinished = this.animationEnabled > 0.0D;
      
      // Default state text color
      Color colorText = ColorUtil.setAlpha(this.animation, ColorUtil.colorAlpha(Color.WHITE, (int)(122.4 + (animationFinished ? 132.6 * this.animationEnabled : 0.0D))));
      
      BuiltText text = (BuiltText)Builder.text().font(FontManager.SUISSEINTMEDIUM.get()).text(displayName).color(colorText).size(7.0F).thickness(0.05F).build();
      text.render(matrix, x + 2.0D, y + 2.0D);
      
      if (!mode.animation().finished(Direction.BACKWARDS)) {
         BuiltRectangle rect = Builder.rectangle().size(new SizeState(width, height)).color(new QuadColorState(ColorUtil.setAlpha(this.animation * mode.animation().getOutput(), new Color(255, 255, 255, 20)))).radius(new QuadRadiusState(2.0F)).smoothness(1.2F).build();
         rect.render(matrix, x, y);
         
         BuiltBorder border = (BuiltBorder)Builder.border().size(new SizeState(width, height)).color(new QuadColorState(ColorUtil.setAlpha(this.animation * mode.animation().getOutput(), Color.WHITE))).radius(new QuadRadiusState(2.0F)).thickness(0.01F).smoothness(0.65F, 0.65F).build();
         border.render(matrix, x, y);
         
         // Highlight state text color
         text = (BuiltText)Builder.text().font(FontManager.SUISSEINTMEDIUM.get()).text(displayName).color(ColorUtil.setAlpha(this.animation * mode.animation().getOutput(), Color.WHITE)).size(7.0F).thickness(0.05F).build();
         text.render(matrix, x + 2.0D, y + 2.0D);
      }
