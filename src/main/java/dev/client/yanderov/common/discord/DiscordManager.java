package dev.client.yanderov.common.discord;

import antidaunleak.api.UserProfile;
import dev.client.yanderov.Yanderov;
import dev.client.yanderov.common.discord.utils.DiscordEventHandlers;
import dev.client.yanderov.common.discord.utils.DiscordYanderovPresence;
import dev.client.yanderov.common.discord.utils.DiscordRPC;
import dev.client.yanderov.common.discord.utils.RPCButton;
import dev.client.yanderov.utils.client.discord.Buffer;
...
public class DiscordManager implements QuickImports {
   ...
   public void init() {
      ...
      if (!os.contains("linux")) {
         DiscordEventHandlers handlers = (new DiscordEventHandlers.Builder()).ready((user) -> {
            YanderovIntegration.getInstance().getDiscordManager().setInfo(new DiscordInfo(user.username, "https://cdn.discordapp.com/avatars/" + user.userId + "/" + user.avatar + ".png", user.userId));
            ...
            DiscordYanderovPresence YanderovPresence = var10000.setState("Uid: " + var10001.profile("uid")).setLargeImage("https://i.postimg.cc/nznMWbhM/0001-0250.gif", "https://t.me/YanderovDLC").setSmallImage(YanderovIntegration.getInstance().getDiscordManager().getInfo().avatarUrl, "https://t.me/YanderovDLC").setButtons(RPCButton.create("Ð¢ÐµÐ»ÐµÐ³Ñ€Ð°Ð¼", "https://t.me/YanderovDLC")).build();
   ...
   private class DiscordDaemonThread extends Thread {
      public void run() {
         ...
         try {
            while(YanderovIntegration.getInstance().getDiscordManager().isRunning()) {
