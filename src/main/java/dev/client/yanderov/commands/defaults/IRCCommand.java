...
public class IRCCommand extends Command {
   ...
   public void execute(String label, IArgConsumer args) throws CommandException {
      IRC ircModule = (IRC)YanderovIntegration.getInstance().getModuleRepository().modules().stream().filter((module) -> module instanceof IRC).map((module) -> (IRC)module).findFirst().orElse((Object)null);
      if (ircModule == null) {
   ...
   private void sendSetPrefix(String prefix) {
      if (YanderovIntegration.getInstance().getIrcManager().getClient() != null && YanderovIntegration.getInstance().getIrcManager().getClient().isOpen()) {
         YanderovIntegration.getInstance().getIrcManager().getClient().sendSetPrefix(prefix);
      }
