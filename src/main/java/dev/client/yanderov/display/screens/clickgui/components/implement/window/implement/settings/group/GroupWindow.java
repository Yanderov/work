package dev.client.yanderov.display.screens.clickgui.components.implement.window.implement.settings.group;
...
public class GroupWindow extends AbstractWindow {
   ...
   public void drawWindow(class_332 context, int mouseX, int mouseY, float delta) {
      ...
      this.height = (float)class_3532.method_15340(this.getComponentHeight(), 0, 200);
      rectangle.render(ShapeProperties.create(matrix, (double)this.x, (double)this.y, (double)(this.width + 30.0F), (double)this.height).round(4.0F).softness(2.0F).thickness(1.0F).outlineColor((new Color(75, 75, 75, 255)).getRGB()).color((new Color(14, 14, 16, 255)).getRGB(), (new Color(31, 27, 35, 255)).getRGB(), (new Color(31, 27, 35, 255)).getRGB(), (new Color(14, 14, 16, 255)).getRGB()).build());
      FontRenderer var10000 = Fonts.getSize(15, Fonts.Type.SEMI);
      class_4587 var10001 = context.method_51448();
      String var10002 = this.setting.getName();
      ...
      for(int i = this.components.size() - 1; i >= 0; --i) {
         ...
         if (visible == null || (Boolean)visible.get()) {
            component.x = this.x;
            component.y = (float)((double)(this.y + 19.0F + offset + ((float)(this.getComponentHeight() - 25) - component.height)) + this.smoothedScroll);
   ...
   public boolean mouseClicked(double mouseX, double mouseY, int button) {
      this.draggable(Calculate.isHovered(mouseX, mouseY, (double)this.x, (double)this.y, (double)this.width, (double)19.0F) && button == 0);
